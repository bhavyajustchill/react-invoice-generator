...
class pdfExport {
  ...
  static setSettings = () => {
    if (localStorage.getItem("defaultSettings")) {
      this.defaultSettings = JSON.parse(localStorage.getItem("defaultSettings"));
    }
  ...
  static getTotal = (items, discounts, shipping) =>
    (this.getSubtotal(items) - Number(discounts || 0) + Number(shipping || 0)).toFixed(2);

  static getTemplateWithDef = (obj) => {
    // TODO: Remove on prod (This for demo purpose only)
    // obj.invoiceTo = "Invoice to Demo"
    // END TODO
    let docDef = null;
  ...
  static generateDocDef = (obj) => {
    const { backgroundColor, watermark, pageSize, pageOrientation, qrCodeActive, qrCode } =
      this.docDefinition;
    let itemBody = [
      [
          ...
          columns: [
            {
              ...
              text: [
                {
                  text: obj.terms !== "" ? "Terms & Conditions\n" + obj.terms : "",
                  fontSize: 10,
                },
              ],
            },
